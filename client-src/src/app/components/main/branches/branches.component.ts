    ...
    this._branchService.getBranches().subscribe(
      data => {
        ...
        for (let i = 0; i < arrayOfPolygons.length; i++) {
          parcel = arrayOfPolygons[i];
          for (let j = 0; j < arrayOfPolygons.length; j++) {
            parce2 = arrayOfPolygons[j];
